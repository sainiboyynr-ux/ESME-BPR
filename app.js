...
client
    ...
    .setProject('6a086db7001ad180a397');

const databases = new Appwrite.Databases(client);
const APPWRITE_DB_ID = '6a0870b50016c92e8890';
const APPWRITE_COL_ID = 'bpr_records';

const urlParams = new URLSearchParams(window.location.search);
let bprId = urlParams.get('id') || 'default';
let STORE_KEY = 'bpr_data_v2_' + bprId;
let currentUser = null;
let bprData = defaultData();
...
async function loadData() {
  ...
  try {
    const doc = await databases.getDocument(APPWRITE_DB_ID, APPWRITE_COL_ID, bprId);
    if (doc && doc.data) {
      bprData = JSON.parse(doc.data);
    }
  } catch (e) {
    console.warn("Could not load from Appwrite, loading default.", e);
    bprData = defaultData();
  }
}
...
async function saveData() {
  ...
  if (bprId !== 'default') {
    try {
      await databases.updateDocument(APPWRITE_DB_ID, APPWRITE_COL_ID, bprId, {
        data: JSON.stringify(bprData)
      });
    } catch(e) {
      console.error('Error saving to Appwrite', e);
      showToast('error', 'Cloud sync failed! ' + (e.message || 'Check console.'));
    }
  }
}
...
async function handleShare() {
  ...
  const newId = bprData.header.batchNo.replace(/[^a-zA-Z0-9._-]/g, '-').toLowerCase().substring(0, 36);
  const shareUrl = window.location.origin + window.location.pathname + '?id=' + newId;
  
  try {
    await databases.createDocument(APPWRITE_DB_ID, APPWRITE_COL_ID, newId, {
      data: JSON.stringify(bprData)
    });
  } catch (e) {
    if (e.code === 409) {
      // Document already exists, update it instead
      await databases.updateDocument(APPWRITE_DB_ID, APPWRITE_COL_ID, newId, {
        data: JSON.stringify(bprData)
      });
    } else {
      console.error('Create error:', e);
      showToast('error', 'Could not create link: ' + (e.message || 'Unknown error'));
      return;
    }
  }
  ...
  navigator.clipboard.writeText(shareUrl).then(() => {
    showModal('🔗', 'Share Link Generated', 'The cloud link has been copied to your clipboard!\n\n' + shareUrl, () => {});
  }).catch(() => {
